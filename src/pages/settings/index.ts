...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#video').on('click', function () {
    $(this)
      .siblings('.dd-child')
      .html(
        '<iframe width="100%" src="https://www.youtube.com/embed/hIJyF2u3-RY" title="Quick Tutorial for SoundCloud Player 1.3.0" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
      );
  });

  $('#eshortcuts').text(
    `To change the shortcut for opening popup, access '${
      isChrome() ? 'chrome://extensions/shortcuts' : 'about:addons'
    }' manually.`
  );
});
...
async function update(val: Record<any, any>) {
  ...
  if (val['title'] != null) {
    $('.title,.breathing').text(
      replaceText(localStorage.getItem('trackdisplay'), val)
    );
    $('.title,.breathing').attr(
      'title',
      replaceText(localStorage.getItem('trackdisplay'), val)
    );
    startMarquees();
...
async function initResetButton() {
  $('#reset').on('click', function () {
    ...
    if (b == 3) {
      // if it's '>=', it's gonna add elements more than 1.
      $('#sure').append(
        $(
          `<div><br>ARE YOU SURE YOU WANT TO RESET EVERYTHING ? [<span id='yes' class='clickable'>YES</span>] [<span id='no' class='clickable'>NO</span>] </div>`
        )
      );
...
function checkFonts() {
  ...
  $('#font-size').val(
    Number(localStorage.getItem('font-size').replace('px', ''))
  );
  ...
  (async () => {
    ...
    $(`#fontlist option[value='${localStorage.getItem('font')}']`).attr(
      'selected',
      'true'
    );
  })();
...
function checkCustomColors() {
  ...
  if (localStorage.getItem('themecolor') != null) {
    ...
    $('#current-theme').val(
      `${localStorage.getItem('themecolor').toUpperCase()}`
    );
    updateThemeColor();
  } else {
    setTimeout(function () {
  ...
  $('#themecolor').change(function () {
    $('#current-theme').val(`${($(this).val() as string).toUpperCase()}`);
    updateThemeColor($(this).val());
  });
  $('#current-theme').change(function () {
    $('#themecolor').val(`${($(this).val() as string).toUpperCase()}`);
    updateThemeColor($(this).val());
  });
  ...
  if (localStorage.getItem('bgcolor') != null) {
    ...
    $('#current-bgcolor').val(
      `${localStorage.getItem('bgcolor').toUpperCase()}`
    );
    updateBGcolor();
  } else {
    setTimeout(function () {
  ...
  $('#bgcolor').change(function () {
    $('#current-bgcolor').val(`${($(this).val() as string).toUpperCase()}`);
    updateBGcolor($(this).val());
  });
  $('#current-bgcolor').change(function () {
    $('#bgcolor').val(`${($(this).val() as string).toUpperCase()}`);
    updateBGcolor($(this).val());
  });
}
...
async function initInputs() {
  ...
  for (let i = 0; i < list.length; i++) {
    (function (n) {
      $(list[n][0]).on('input', function () {
        localStorage.setItem(list[n][1], $(this).val() as string);
      });
    })(i);
  }
  ...
  for (let i = 0; i < checkboxes.length; i++) {
    (function (n) {
      $<HTMLInputElement>(checkboxes[n][0]).change(function () {
        localStorage.setItem(checkboxes[n][1], this.checked + '');
      });

      if (
        localStorage.getItem(checkboxes[n][1]) != null &&
        localStorage.getItem(checkboxes[n][1]) == 'true'
      ) {
...
async function registerEvents() {
  ...
  $<HTMLInputElement>('#toggle-compact').change(function () {
    if (this.checked) {
      startMarquees();
      $('#controller-body').css('display', 'inline-block');
    } else {
  ...
  $<HTMLInputElement>('#display-artwork').change(function () {
    toggleArtwork(this.checked);
  });
}
...
async function initDarkModeAutomation() {
  ...
  for (let i = 0; i < el.length; i++) {
    $(el[i]).on('input', function () {
      let o = $(this)[0].id,
        head = o.includes('s') ? 'range-start' : 'range-end',
        num = o.includes('h') ? 0 : 1,
        z = Number($(this).val());
      auto[head][num] = z;
